...
Entries above the dividing line are this fork's. Entries below are inherited
from the upstream sitapix/govee2mqtt git-cliff history.

## [2026.05.03] - 2026-05-03

### Added

- 2FA / email-verification login support: handles Govee server-side status 454/455.
- Auto-trigger of `/v1/verification` so the verification email is sent without
  manual API calls.
- New add-on option `govee_2fa_code` (also `GOVEE_2FA_CODE` env var) for
  one-shot code injection. Token is cached after first successful login.
- Updated `APP_VERSION` to `7.4.10` and switched login to `/v2/login` with
  the headers Govee currently expects.

### Fixed

- `addon/Dockerfile` now pulls `ghcr.io/fat-fred/govee2mqtt:latest` (this
  fork's binary) instead of upstream sitapix.
- `scripts/apply-tag.sh` honours the actual git tag when running in CI so
  the addon version matches the pushed tag.
- `addon/run.sh` no longer leaks future env vars via `set -x`; redaction
  regex covers MQTT user/password as well.

### Note

This is a temporary fork. When upstream `wez/govee2mqtt` merges 2FA support,
switch back. No new non-2FA features will land here.
